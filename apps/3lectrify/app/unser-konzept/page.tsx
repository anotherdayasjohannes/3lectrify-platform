...
async function getUnserKonzeptPage() {
  const page = await client.fetch(pageQuery, { slug: 'unser-konzept' });
  return page;
}

export async function generateMetadata() {
  const page = await getUnserKonzeptPage();
  
  return {
    title: page?.title || 'Unser Konzept | 3lectrify',
    description: page?.description || 'Erfahren Sie mehr über unser innovatives Konzept für nachhaltige Immobilienentwicklung.',
  };
}

export default async function UnserKonzeptPage() {
  const page = await getUnserKonzeptPage();
  ...
  if (!page) {
    return (
      <div style={{ padding: '100px 20px', textAlign: 'center' }}>
        ...
        <p>Create a page with slug &quot;unser-konzept&quot; in Sanity Studio</p>
      </div>
    );
  }
